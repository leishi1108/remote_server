...
class SequenceMatchAgent:
    def __init__(self, text_key, model, tools=None, prompt=None):
        ...
        self.model = model
        self.prompt = prompt if prompt is not None else "你是建筑行业的专家，请根据输入文本内容选择匹配的做法层。\n匹配要求：\n1、如果没有匹配的做法层就返回空列表，不需要解释原因；\n2、输出是匹配的做法层的列表，必须在下列做法层集合中: [加固处理, 勾缝处理, 涂料喷刷, 磨光, 打蜡, 防滑处理, 裱糊处理, 刚性层处理, 排气处理, 排水处理, 垫层处理, 基层处理, 回填, 钢筋焊接, 植筋, 塞口处理, 龙骨安装, 嵌缝, 吊杆安装, 防腐处理, 抹灰, 防锈处理, 隔离处理, 底层处理, 找平, 找坡, 结合处理, 保温处理, 防水处理, 隔气处理, 防护处理, 面层处理, 涂料喷刷\中层漆喷刷, 涂料喷刷\底漆喷刷, 回填\房心回填, 面层处理\块料挂贴, 面层处理\块料挂贴\干挂, 面层处理\块料挂贴\湿挂]；"
